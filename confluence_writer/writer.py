...
class ConfluenceWriter:
    ...
    def collect(
        ...
        *,
            ...
            if mapping.effective_selection == "subtree":
                ...
                for root_id in mapping.selected_page_ids:
                    try:
                        ...
                        # A failed root hides an unknown set of descendants. Counting
                        # only the root as failed cannot authorize their deletion.
                        raise ConfluenceWriterError(
                            "Subtree enumeration is incomplete; preserving the current generation."
                        ) from exc
        ...
        changed = [
            ...
            or previous_revisions[page.page_id] != self._materialization_revision(
                mappings[page.space_key], page
            )
        ]
        ...
        for mapping in self._settings.spaces:
            ...
            if (
                zone_uid not in current_ids
                or previous_revisions.get(zone_uid) != self._materialization_revision(mapping)
            ):
                ...
                for converted in batch.converted:
                    ...
                    documents.append(
                        CollectedDocument(
                            ...
                            artifacts=artifacts,
                            source_revision=self._materialization_revision(mapping, page),
                        )
                    )
                failures.extend(
    ...
    def _materialization_revision(
        self, mapping: SpaceMapping, page: RemotePage | None = None,
    ) -> str:
        """Invalidate generated bytes when remote content or local mapping changes."""
        payload = {
            "base_url": self._settings.base_url,
            "space_key": mapping.space_key,
            "target": mapping.target,
            "classification": mapping.classification,
            "remote_revision": None if page is None else _source_revision(page),
        }
        encoded = json.dumps(payload, sort_keys=True, separators=(",", ":")).encode("utf-8")
        return "materialization-v1:" + hashlib.sha256(encoded).hexdigest()
    ...
    def _zone_readme(self, mapping: SpaceMapping, observed_at: datetime) -> CollectedDocument:
        ...
        return CollectedDocument(
            ...
            source_revision=self._materialization_revision(mapping),
        )
